...
#include "netgauge.h"
#ifdef NG_PTRN_OVERLAP
#include "hrtimer/hrtimer.h"
#include <time.h>
#include "fullresult.h"
#include "statistics.h"
#include "ptrn_overlap_cmdline.h"

static struct ptrn_overlap_cmd_struct args_info;

extern struct ng_options g_options;

/* internal function prototypes */
static void overlap_do_benchmarks(struct ng_module *module);

/**
 * comm. pattern description and function pointer table
 */
static struct ng_comm_pattern pattern_overlap = {
   .name = "overlap",
   .desc = "measures overlap ....",
   .flags = 0,
   .do_benchmarks = overlap_do_benchmarks
};
...
int register_pattern_overlap() {
   ng_register_pattern(&pattern_overlap);
   return 0;
}
...
static void overlap_do_benchmarks(struct ng_module *module) {
	...
	if (ptrn_overlap_parser_string(g_options.ptrnopts, &args_info, "netgauge") != 0) {
		ng_abort("commandline parser error");
	}

  printf("hello %i %i %i\n", g_options.testcount, g_options.min_datasize, args_info.input_arg);
...
#else
int register_pattern_overlap(void) {return 0;};
#endif
