...
#include "netgauge.h"
#if defined NG_PTRN_ONE_ONE_SYNC && defined NG_MPI
 ...
 */
static struct ng_comm_pattern pattern_one_one_sync = {
   pattern_one_one_sync.name = "one_one_sync",
   pattern_one_one_sync.desc = "measures ping-pong latency&bandwidth (MPI_Ssend)",
   pattern_one_one_sync.flags = 0,
   pattern_one_one_sync.do_benchmarks = one_one_do_benchmarks
};
...
int register_pattern_one_one_sync() {
   ng_register_pattern(&pattern_one_one_sync);
   return 0;
}
    ...
    for (int test = -1 /* 1 warmup test */; test < test_count; test++) {
      ...
      /* TODO: add cool abstract dot interface ;) */
	    ...
	    if (rank==1) {
          ...
          MPI_Ssend(buffer, data_size, MPI_BYTE, 0 /*dst*/, 0 /*tag*/, MPI_COMM_WORLD);
          ...
          //NG_SEND(g_options.mpi_opts->partner, buffer, data_size, module);
          MPI_Ssend(buffer, data_size, MPI_BYTE, 1 /*src*/, 0 /*tag*/, MPI_COMM_WORLD);
          ...
          //NG_RECV(g_options.mpi_opts->partner, buffer, data_size, module);
          MPI_Recv(buffer, data_size, MPI_BYTE, 1 /*dst*/, 0 /*tag*/, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
#else
extern "C" {
int register_pattern_one_one_sync(void) {return 0;};
}
#endif
