...
#include "netgauge.h"
#if defined NG_PTRN_ONE_ONE_RANDTAG
...
/* TODO: this module is very much the same as the one_one ... it's a
 * hack ;) ... however, it only works with MPI and can't use the NG
 * module interface (THIS IS AN ABSTRACTION BREAK). We should extend NG
 * modules with tags if they are available .... later */
...
static void one_one_randtag_do_benchmarks(struct ng_module *module);

/**
 * comm. pattern description and function pointer table
 */
static struct ng_comm_pattern pattern_one_one_randtag = {
   pattern_one_one_randtag.name = "one_one_randtag",
   pattern_one_one_randtag.desc = "one_one with random tags and MPI_ANY_TAG",
   pattern_one_one_randtag.flags = 0,
   pattern_one_one_randtag.do_benchmarks = one_one_randtag_do_benchmarks
};
...
int register_pattern_one_one_randtag() {
   ng_register_pattern(&pattern_one_one_randtag);
   return 0;
}
...
static void one_one_randtag_do_benchmarks(struct ng_module *module) {
  ...
  if(strstr(module->name, "mpi") != module->name) {
    ng_abort("the one_one_randtag pattern works only with module \"mpi\"\n");
  }
    ...
    for (int test = -1 /* 1 warmup test */; test < test_count; test++) {
      ...
      randtag = mtrand.randInt(32768);
	    if (rank==1) {
          ...
          MPI_Recv(buffer, data_size, MPI_BYTE, g_options.mpi_opts->partner, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

          /* Phase 2: send data back */
          //NG_SEND(g_options.mpi_opts->partner, buffer, data_size, module);
          MPI_Send(buffer, data_size, MPI_BYTE, g_options.mpi_opts->partner, randtag, MPI_COMM_WORLD);
...
#else 
extern "C" {
int register_pattern_one_one_randtag(void) {return 0;};
} /* extern C */
#endif
